...
(function(global, factory) {
  ...
  function LEFT(text, number) {
    var someError = anyError(text, number);
    if (someError) {
      return someError;
    }
  ...
  function ATAN2(number_x, number_y) {
    ...
    var anyError$1 = anyError(number_x, number_y);
    if (anyError$1) {
      return anyError$1;
    }
  ...
  function DECIMAL(number, radix) {
    ...
    number = parseNumber(number);
    radix = parseNumber(radix);
    var anyError$1 = anyError(number, radix);
    if (anyError$1) {
      return anyError$1;
    }
    if (radix === 0) {
  ...
  function ROUND(number, digits) {
    ...
    var anyError$1 = anyError(number, digits);
    if (anyError$1) {
      return anyError$1;
    }
  ...
  function CUMIPMT(rate, periods, value$1, start, end, type) {
    ...
    var payment = PMT(rate, periods, value$1, 0, type);
    var interest = 0;
    if (start === 1) {
      if (type === 0) {
        interest = -value$1;
  ...
  function NPER(rate, payment, present, future, type) {
    ...
    if (rate === 0) {
      ...
      var _num = payment * (1 + rate * type) - future * rate;
  ...
  function LOOKUP(searchCriterion, array, resultArray) {
    ...
    for (var i = 0; i < array.length; i++) {
      ...
      } else if (isNumberLookup && array[i] <= searchCriterion || typeof array[i] === "string" && array[i].localeCompare(searchCriterion) < 0) {
  ...
  function INDEX(cellRange, rowNumber, columnNumber) {
    var someError = anyError(cellRange, rowNumber, columnNumber);
    if (someError) {
      return someError;
    }
